...
@section('content')
    <form class="create_form_doc" action="{{ route('chunks.update', $chunk->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="form_scroll_all">
            <div class="form_row">
                <div class="form-group">
                    <label >
                        {{__('name')}}
                        <input value="{{ old('name', $chunk->name) }}" placeholder="{{ $chunk->name }}" type="text" name="name" class="form-control" required>
                    </label>
                </div>
            </div>
